...
from robopianist.suite.tasks import piano_with_hands, PianoWithHands
from robopianist.suite.tasks.cube_task import CubeTask
...
def load(
    ...
    return composer_utils.Environment(
        task=CubeTask(midi=midi, **task_kwargs) if cube else PianoWithHands(midi=midi, **task_kwargs),
        random_state=seed,
