# Copyright 2023 The RoboPianist Authors.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""A task where two shadow hands must play a given MIDI file on a piano."""
from typing import List, Optional, Sequence, Tuple, Dict, Callable

import numpy as np
from dm_control import mjcf
from dm_control.composer import variation as base_variation
from dm_control.composer.observation import observable
from dm_control.mjcf import commit_defaults
from dm_control.utils.rewards import tolerance
from dm_env import specs
from mujoco_utils import collision_utils, spec_utils, mjcf_utils

from configs import DRConfig, JointConfig
from robopianist.models.arenas import stage
from robopianist.models.arm import robot_arm_constants
from robopianist.models.arm.robot_arm import RobotArm
from robopianist.models.hands import ShadowHand, Hand, shadow_hand_constants, allegro_hand_constants, AllegroHand
from robopianist.music import midi_file
from robopianist.suite import composite_reward
from robopianist.suite.tasks import base

# Distance thresholds for the shaping reward.
_HAND_CLOSE_ENOUGH_TO_KEYS = 0.03
_FINGER_CLOSE_ENOUGH_TO_KEY = 0.01
_KEY_CLOSE_ENOUGH_TO_PRESSED = 0.05

# Transparency of fingertip geoms.
_FINGERTIP_ALPHA = 1.0


class PianoWithHands(base.PianoTask):
    def __init__(
            self,
            midi: midi_file.MidiFile,
            n_steps_lookahead: int = 1,
            n_seconds_lookahead: Optional[float] = None,
            trim_silence: bool = False,
            wrong_press_termination: bool = False,
            initial_buffer_time: float = 0.0,
            disable_colorization: bool = False,
            disable_hand_collisions: bool = False,
            augmentations: Optional[Sequence[base_variation.Variation]] = None,
            energy_penalty_coef: float = 0.0,
            action_penalty_coef: float = 0.0,
            sliding_penalty_coef: float = 0.0,
            randomization_config: DRConfig = None,
            hand_class: type(Hand) = ShadowHand,
            reward_functions: Sequence[str] = (),
            **kwargs,
    ) -> None:
        """Task constructor.

        Args:
            midi: A `MidiFile` object.
            n_steps_lookahead: Number of timesteps to look ahead when computing the
                goal state.
            n_seconds_lookahead: Number of seconds to look ahead when computing the
                goal state. If specified, this will override `n_steps_lookahead`.
            trim_silence: If True, shifts the MIDI file so that the first note starts
                at time 0.
            wrong_press_termination: If True, terminates the episode if the hands press
                the wrong keys at any timestep.
            initial_buffer_time: Specifies the duration of silence in seconds to add to
                the beginning of the MIDI file. A non-zero value can be useful for
                giving the agent time to place its hands near the first notes.
            disable_fingering_reward: If True, disables the shaping reward for
                fingering. This will also disable the colorization of the fingertips
                and corresponding keys. Note that if the MIDI file does not contain
                any fingering information, the fingering reward will also be disabled.
            disable_forearm_reward: If True, disables the shaping reward for the
                forearms.
            disable_colorization: If True, disables the colorization of the fingertips
                and corresponding keys.
            disable_hand_collisions: If True, disables collisions between the two hands.
            augmentations: A list of `Variation` objects that will be applied to the
                MIDI file at the beginning of each episode. If None, no augmentations
                will be applied.
            energy_penalty_coef: Coefficient for the energy penalty.
            randomize_hand_positions: If True, randomizes the initial position of the
                hands at the beginning of each episode.
        """
        super().__init__(arena=stage.Stage(), hand_class=hand_class, **kwargs)

        if trim_silence:
            midi = midi.trim_silence()
        self._midi = midi
        self._initial_midi = midi
        self._n_steps_lookahead = n_steps_lookahead
        if n_seconds_lookahead is not None:
            self._n_steps_lookahead = int(
                np.ceil(n_seconds_lookahead / self.control_timestep)
            )
        self._initial_buffer_time = initial_buffer_time

        self._set_rewards(reward_functions)
        self._has_fingering_reward = self._compute_fingering_reward in self._selected_rewards.values()
        self._has_fingering_reward = self._has_fingering_reward and self._midi.has_fingering()

        self._wrong_press_termination = wrong_press_termination
        self._disable_colorization = disable_colorization
        self._disable_hand_collisions = disable_hand_collisions
        self._augmentations = augmentations
        self._energy_penalty_coef = energy_penalty_coef
        self._action_penalty_coef = action_penalty_coef
        self._sliding_penalty_coef = sliding_penalty_coef
        self._randomization_config = randomization_config

        if self._has_fingering_reward and not disable_colorization:
            self._colorize_fingertips(hand_class)
        if disable_hand_collisions:
            self._disable_collisions_between_hands()
        self._reset_quantities_at_episode_init()
        self._reset_trajectory()  # Important: call before adding observables.
        self._add_observables()
        self.last_ctrl = 0
        self.basis_dampings = None

    def _set_rewards(self, reward_names: Sequence[str]) -> None:
        available_reward_functions: Dict[str, Callable] = {
            "energy_reward": self._compute_energy_reward,
            "action_reward": self._compute_action_reward,
            "sliding_reward": self._compute_sliding_reward,

            "hand_reward": self._compute_hand_reward,

            "friendly_keypress_reward": self._compute_friendly_key_press_reward,
            "keypress_reward": self._compute_key_press_reward,
            "keypress_reward_linear": self._compute_key_press_reward_linear,
            "keypress_reward_complex": self._compute_key_press_reward_complex,
            "keypress_reward_original": self._compute_key_press_reward_original,

            "fingering_reward": self._compute_fingering_reward,
            "forearm_reward": self._compute_forearm_reward,

        }
        self._selected_rewards = {k: v for k, v in available_reward_functions.items() if k in reward_names}
        self._reward_fn = composite_reward.CompositeReward(**self._selected_rewards)

    def _reset_quantities_at_episode_init(self) -> None:
        self._t_idx: int = 0
        self._should_terminate: bool = False
        self._discount: float = 1.0

    def _maybe_change_midi(self, random_state: np.random.RandomState) -> None:
        if self._augmentations is not None:
            midi = self._initial_midi
            for var in self._augmentations:
                midi = var(initial_value=midi, random_state=random_state)
            self._midi = midi
            self._reset_trajectory()

    def _reset_trajectory(self) -> None:
        note_traj = midi_file.NoteTrajectory.from_midi(
            self._midi, self.control_timestep
        )
        note_traj.add_initial_buffer_time(self._initial_buffer_time)
        self._notes = note_traj.notes

    # Composer methods.

    def initialize_episode(
            self, physics: mjcf.Physics, random_state: np.random.RandomState
    ) -> None:
        self._maybe_change_midi(random_state)
        self._reset_quantities_at_episode_init()
        self._apply_domain_randomization(physics, random_state)

        if self.hand_class == RobotArm:
            ids = np.array(physics.model.actuator_trnid).transpose()[0]
            for side_ids in np.split(ids, len(self.hands)):
                for i, init_pos in zip(side_ids, robot_arm_constants.INITIAL_JOINT_STATES):
                    physics.data.qpos[i] = init_pos

    def before_step(
            self,
            physics: mjcf.Physics,
            action: np.ndarray,
            random_state: np.random.RandomState,
    ) -> None:
        """Applies the control to the hands"""
        hand_actions = np.split(action[:], len(self.hands))
        for hand, actions in zip(self.hands, hand_actions):
            hand.apply_action(physics, actions, random_state)

    def notify_episode(self, episode: int):
        for hand in self.hands:
            hand.notify_episode(episode)

    @property
    def action_labels(self) -> list[str]:
        labels = []
        for hand in self.hands:
            labels.extend(hand.action_labels)
        return labels + self.piano.action_labels

    def after_step(
            self, physics: mjcf.Physics, random_state: np.random.RandomState
    ) -> None:
        del random_state  # Unused.
        self._t_idx += 1
        self._should_terminate = (self._t_idx - 1) == len(self._notes) - 1

        self._goal_current = self._goal_state[0]

        has_hand_reward = self._compute_hand_reward in self._selected_rewards.values()
        if self._has_fingering_reward or has_hand_reward or not self._disable_colorization:
            self._update_fingering_state()
            self._rh_keys_current = self._rh_keys
            self._lh_keys_current = self._lh_keys

        if not self._disable_colorization:
            self._colorize_keys(physics)

        should_not_be_pressed = np.flatnonzero(1 - self._goal_current[:-1])
        self._failure_termination = self.piano.activation[should_not_be_pressed].any()

    def get_reward(self, physics: mjcf.Physics) -> float:
        return self._reward_fn.compute(physics)

    def get_discount(self, physics: mjcf.Physics) -> float:
        del physics  # Unused.
        return self._discount

    def should_terminate_episode(self, physics: mjcf.Physics) -> bool:
        del physics  # Unused.
        if self._should_terminate:
            return True
        if self._wrong_press_termination and self._failure_termination:
            self._discount = 0.0
            return True
        return False

    @property
    def task_observables(self):
        return self._task_observables

    def action_spec(self, physics: mjcf.Physics) -> specs.BoundedArray:
        return spec_utils.merge_specs([hand.action_spec(physics) for hand in self.hands])

    # Other.

    @property
    def midi(self) -> midi_file.MidiFile:
        return self._midi

    @property
    def reward_fn(self) -> composite_reward.CompositeReward:
        return self._reward_fn

    # Helper methods.

    def _compute_forearm_reward(self, physics: mjcf.Physics) -> float:
        """Reward for not colliding the forearms."""
        if collision_utils.has_collision(
                physics,
                [g.full_identifier for g in self.right_hand.root_body.geom],
                [g.full_identifier for g in self.left_hand.root_body.geom],
        ):
            return 0.0
        return 0.5

    def _compute_energy_reward(self, physics: mjcf.Physics) -> float:
        """Reward for minimizing energy."""
        rew = 0.0
        for hand in self.hands:
            power = hand.observables.actuators_power(physics).copy()
            weights = np.ones(power.shape)
            weights[-1] = 0.003
            rew -= self._energy_penalty_coef * np.average(power, weights=weights)
        return rew

    def _compute_action_reward(self, physics: mjcf.Physics) -> float:
        """Reward for minimizing fast movements."""
        ctrl = []
        for hand in self.hands:
            ctrl.extend(physics.bind(hand.actuators).ctrl)
        ctrl = np.array(ctrl)

        diff = np.power(ctrl - self.last_ctrl, 2)
        rew = 1 - self._action_penalty_coef * np.sum(diff)
        self.last_ctrl = ctrl
        return rew

    def _compute_sliding_reward(self, physics: mjcf.Physics) -> float:
        """Penalize sliding while keys are pressed."""

        pressed = self.piano.activation[:].astype(int)
        pressed += np.roll(pressed, 1)
        pressed = pressed.max()

        velocities = []
        for hand in self.hands:
            velocities.append(physics.bind(hand.joints[-1]).qvel)
        velocities = np.array(velocities)
        diff = np.power(velocities, 2)

        rew = 1 - pressed * np.sum(diff) * self._sliding_penalty_coef
        return rew

    def _compute_key_press_reward(self, physics: mjcf.Physics) -> float:
        """Reward for pressing the right keys at the right time."""
        del physics  # Unused.
        on = np.flatnonzero(self._goal_current[:-1])
        # It's possible we have no keys to press at this timestep, so we need to check
        # that `on` is not empty.
        if on.size > 0:
            should_be_pressed = np.flatnonzero(self._goal_current[:-1])
            correct_key_presses = self.piano.activation[should_be_pressed].sum()

            should_not_be_pressed = np.flatnonzero(1 - self._goal_current[:-1])
            wrong_key_presses = self.piano.activation[should_not_be_pressed].sum()

            amount_pressed = np.mean(self.piano.normalized_state[should_be_pressed])

            if correct_key_presses == 0 and wrong_key_presses == 0:
                return 0
            elif wrong_key_presses > 0:
                return 0.5 + 0.5 * amount_pressed
            elif correct_key_presses > 0 and wrong_key_presses == 0:
                return 1 + 0.5 * amount_pressed
        else:
            if self.piano.activation.any():
                return 2 * (1 - np.max(self.piano.normalized_state))
            else:
                return 1.5

    def _compute_key_press_reward_linear(self, physics: mjcf.Physics) -> float:
        """Reward for pressing the right keys at the right time."""
        del physics  # Unused.
        on = np.flatnonzero(self._goal_current[:-1])
        rew = 0.0
        # It's possible we have no keys to press at this timestep, so we need to check
        # that `on` is not empty.
        if on.size > 0:
            if self.piano.activation.any():
                should_be_pressed = np.flatnonzero(self._goal_current[:-1])
                correct_key_presses = self.piano.activation[should_be_pressed].sum()

                should_not_be_pressed = np.flatnonzero(1 - self._goal_current[:-1])
                wrong_key_presses = self.piano.activation[should_not_be_pressed].sum()

                if correct_key_presses > 0:
                    rew += correct_key_presses * 0.5 + wrong_key_presses * -0.1
                else:
                    rew += -0.5
            else:
                rew += -1.5

            return (rew + 1.5) / 1.8
        else:
            if self.piano.activation.any():
                rew += self.piano.activation.sum() * -0.2
            else:
                rew += 0.3
            return rew + 0.7

    def _compute_friendly_key_press_reward(self, physics: mjcf.Physics) -> float:
        return self._compute_key_press_reward_complex(physics, True)

    def _compute_key_press_reward_complex(self, physics: mjcf.Physics, friendly=False) -> float:
        """Reward for pressing the right keys at the right time."""
        del physics  # Unused.
        on = np.flatnonzero(self._goal_current[:-1])
        rew = 0.0
        # It's possible we have no keys to press at this timestep, so we need to check
        # that `on` is not empty.
        if on.size > 0:
            actual = np.array(self.piano.state / self.piano._qpos_range[:, 1])
            rews = tolerance(
                self._goal_current[:-1][on] - actual[on],
                bounds=(0, _KEY_CLOSE_ENOUGH_TO_PRESSED),
                margin=(_KEY_CLOSE_ENOUGH_TO_PRESSED * 10),
                sigmoid="gaussian",
            )
            rew += 0.5 * rews.mean()

            # No Key press -> penalty
            if friendly and not self.piano.activation.any():
                rew -= 0.5

        if friendly:
            # Less wrong -> better
            off = np.flatnonzero(1 - self._goal_current[:-1])
            wrong_key_presses = self.piano.activation[off].sum()
            rew += 0.3 * (1 - np.minimum(wrong_key_presses / 10.0, 1))
        else:
            # If there are any false positives, the remaining 0.5 reward is lost.
            off = np.flatnonzero(1 - self._goal_current[:-1])
            rew += 0.5 * (1 - float(self.piano.activation[off].any()))

        return rew

    def _compute_key_press_reward_original(self, physics: mjcf.Physics) -> float:
        """Reward for pressing the right keys at the right time."""
        del physics  # Unused.
        on = np.flatnonzero(self._goal_current[:-1])
        rew = 0.0
        # It's possible we have no keys to press at this timestep, so we need to check
        # that `on` is not empty.
        if on.size > 0:
            actual = np.array(self.piano.state / self.piano._qpos_range[:, 1])
            rews = tolerance(
                self._goal_current[:-1][on] - actual[on],
                bounds=(0, _KEY_CLOSE_ENOUGH_TO_PRESSED),
                margin=(_KEY_CLOSE_ENOUGH_TO_PRESSED * 10),
                sigmoid="gaussian",
            )
            rew += 0.5 * rews.mean()
        # If there are any false positives, the remaining 0.5 reward is lost.
        off = np.flatnonzero(1 - self._goal_current[:-1])
        rew += 0.5 * (1 - float(self.piano.activation[off].any()))
        return rew

    def _compute_hand_reward(self, physics: mjcf.Physics):
        def fast_norm(x):
            return np.sqrt(x.dot(x))

        def _distance_hand_to_key(
                hand_keys: List[Tuple[int, int]], hand
        ) -> list[float]:
            if hand is None:
                return []

            if hand.root_body.name == "palm":
                hand_geom = hand.root_body
            else:
                hand_geom = mjcf_utils.safe_find(hand.root_body, "body", "palm")
            hand_pos = np.array(physics.bind(hand_geom).xpos.copy()[0:3])

            # No Keys to be pressed -> distance to line
            if len(hand_keys) == 0:
                key_geom = self.piano.keys[0].geom[0]
                fix_point = np.array(physics.bind(key_geom).xpos.copy()[0:3])
                direction_vector = np.array([0, 1, 0])
                distance = fast_norm(np.cross(hand_pos - fix_point, direction_vector))
                return [distance]

            key_pos = []
            for key, _ in hand_keys:
                key_geom = self.piano.keys[key].geom[0]
                key_geom_pos = physics.bind(key_geom).xpos.copy()[0:3]
                key_pos.append(np.array(key_geom_pos))
            distance_vector = np.mean(key_pos, axis=0) - hand_pos

            # self.set_cube(hand_pos)
            return [fast_norm(distance_vector)]

        if self.right_hand is None or self.left_hand is None:
            target_keys = self._rh_keys_current + self._lh_keys_current
            distances = (_distance_hand_to_key(target_keys, self.right_hand)
                         + _distance_hand_to_key(target_keys, self.left_hand))
        else:
            distances = (_distance_hand_to_key(self._rh_keys_current, self.right_hand)
                         + _distance_hand_to_key(self._lh_keys_current, self.left_hand))
        # Case where there are no keys to press at this timestep.
        if len(distances) == 0:
            return 0.8

        rews = tolerance(
            np.array(distances),
            bounds=(0, _HAND_CLOSE_ENOUGH_TO_KEYS),
            margin=(_HAND_CLOSE_ENOUGH_TO_KEYS * 10),
            sigmoid="gaussian",
        )
        return float(np.mean(rews))

    def _compute_fingering_reward(self, physics: mjcf.Physics) -> float:
        """Reward for minimizing the distance between the fingers and the keys."""

        def _distance_finger_to_key(
                hand_keys: List[Tuple[int, int]], hand
        ) -> List[float]:
            if hand is None:
                return []

            distances = []
            for key, mjcf_fingering in hand_keys:
                fingertip_site = hand.fingertip_sites[mjcf_fingering]
                fingertip_pos = physics.bind(fingertip_site).xpos.copy()
                key_geom = self.piano.keys[key].geom[0]
                key_geom_pos = physics.bind(key_geom).xpos.copy()
                key_geom_pos[-1] += 0.5 * physics.bind(key_geom).size[2]
                key_geom_pos[0] += 0.35 * physics.bind(key_geom).size[0]
                diff = key_geom_pos - fingertip_pos
                distances.append(float(np.linalg.norm(diff)))
            return distances

        distances = _distance_finger_to_key(self._rh_keys_current, self.right_hand)
        distances += _distance_finger_to_key(self._lh_keys_current, self.left_hand)

        # Case where there are no keys to press at this timestep.
        if not distances:
            return 0.0

        rews = tolerance(
            np.hstack(distances),
            bounds=(0, _FINGER_CLOSE_ENOUGH_TO_KEY),
            margin=(_FINGER_CLOSE_ENOUGH_TO_KEY * 10),
            sigmoid="gaussian",
        )
        return float(np.mean(rews))

    def _update_goal_state(self) -> None:
        # Observable callables get called after `after_step` but before
        # `should_terminate_episode`. Since we increment `self._t_idx` in `after_step`,
        # we need to guard against out of bounds indexing. Note that the goal state
        # does not matter at this point since we are terminating the episode and this
        # update is usually meant for the next timestep.
        if self._t_idx == len(self._notes):
            return

        self._goal_state = np.zeros(
            (self._n_steps_lookahead + 1, self.piano.n_keys),
            dtype=np.float64,
        )
        t_start = self._t_idx
        t_end = min(t_start + self._n_steps_lookahead + 1, len(self._notes))
        for i, t in enumerate(range(t_start, t_end)):
            keys = [note.key for note in self._notes[t]]
            self._goal_state[i, keys] = 1.0

    def _update_fingering_state(self) -> None:
        if self._t_idx == len(self._notes):
            return

        fingering = [note.fingering for note in self._notes[self._t_idx]]
        fingering_keys = [note.key for note in self._notes[self._t_idx]]

        # Split fingering into right and left hand.
        self._rh_keys: List[Tuple[int, int]] = []
        self._lh_keys: List[Tuple[int, int]] = []
        for key, finger in enumerate(fingering):
            piano_key = fingering_keys[key]
            if finger < 5:
                self._rh_keys.append((piano_key, finger))
            else:
                self._lh_keys.append((piano_key, finger - 5))

        # For each hand, set the finger to 1 if it is used and 0 otherwise.
        self._fingering_state = np.zeros((2, 5), dtype=np.float64)
        for hand, keys in enumerate([self._rh_keys, self._lh_keys]):
            for key, mjcf_fingering in keys:
                self._fingering_state[hand, mjcf_fingering] = 1.0

    def _add_observables(self) -> None:
        # Enable hand observables.
        enabled_observables = [
            "joints_pos",
            # NOTE(kevin): This observable was previously enabled but it is redundant
            # since it is encoded in the joint positions, specifically via the forearm
            # slider joints (which are in units of meters).
            # "position",
            # "fingertip_force",
        ]
        for hand in self.hands:
            for obs in enabled_observables:
                getattr(hand.observables, obs).enabled = True

        # This returns the current state of the piano keys.
        self.piano.observables.state.enabled = False
        self.piano.observables.activation.enabled = True

        # This returns the goal state for the current timestep and n steps ahead.
        def _get_goal_state(physics) -> np.ndarray:
            del physics  # Unused.
            self._update_goal_state()
            return self._goal_state.ravel()

        goal_observable = observable.Generic(_get_goal_state)
        goal_observable.enabled = True
        self._task_observables = {"goal": goal_observable}

        # This adds fingering information for the current timestep.
        def _get_fingering_state(physics) -> np.ndarray:
            del physics  # Unused.
            self._update_fingering_state()
            return self._fingering_state.ravel()

        has_hand_reward = self._compute_hand_reward in self._selected_rewards.values()
        if self._has_fingering_reward or has_hand_reward or not self._disable_colorization:
            fingering_observable = observable.Generic(_get_fingering_state)
            fingering_observable.enabled = False
            self._task_observables["fingering"] = fingering_observable

    def _colorize_fingertips(self, hand_class: type(Hand)) -> None:
        if hand_class == ShadowHand:
            hand_consts = shadow_hand_constants
        elif hand_class == AllegroHand:
            hand_consts = allegro_hand_constants
        """Colorize the fingertips of the hands."""
        for hand in self.hands:
            for i, body in enumerate(hand.fingertip_bodies):
                color = hand_consts.FINGERTIP_COLORS[i] + (_FINGERTIP_ALPHA,)
                for geom in body.find_all("geom"):
                    if geom.dclass.dclass == "plastic_visual":
                        geom.rgba = color
                # Also color the fingertip sites.
                hand.fingertip_sites[i].rgba = color

    def _colorize_keys(self, physics) -> None:
        """Colorize the keys by the corresponding fingertip color."""
        for hand, keys in zip(
                self.hands,
                [self._rh_keys_current, self._lh_keys_current],
        ):
            for key, mjcf_fingering in keys:
                key_geom = self.piano.keys[key].geom[0]
                if not self.piano.activation[key]:
                    physics.bind(key_geom).rgba = (1.0, 0.5, 0.5, 1.0)

    def _disable_collisions_between_hands(self) -> None:
        """Disable collisions between the hands."""
        for hand in self.hands:
            for geom in hand.mjcf_model.find_all("geom"):
                # If both hands have the same contype and conaffinity, then they can't
                # collide. They can still collide with the piano since the piano has
                # contype 0 and conaffinity 1. Lastly, we make sure we're not changing
                # the contype and conaffinity of the hand geoms that are already
                # disabled (i.e., the visual geoms).
                commit_defaults(geom, ["contype", "conaffinity"])
                if geom.contype == 0 and geom.conaffinity == 0:
                    continue
                geom.conaffinity = 0
                geom.contype = 1

    def _apply_domain_randomization(
            self, physics: mjcf.Physics, random_state: np.random.RandomState
    ) -> None:
        offsets = self._randomization_config
        if offsets is None:
            return

        # Randomize the initial position of the hands.
        offset = random_state.uniform(low=-offsets.hand_pos_offset, high=offsets.hand_pos_offset)
        for hand in self.hands:
            physics.bind(hand.joints[-1]).qpos = offset

        # Randomize the initial position of the piano.
        offset = random_state.uniform(low=-offsets.piano_pos_offset, high=offsets.piano_pos_offset)
        self.piano.set_pose(physics, (0, 0, offset))  # (back, right, up)

        # Randomize joints
        self._apply_joint_randomization(physics, random_state)

        # Randomize activation threshold
        offset = random_state.uniform(low=offsets.activation_offset[0], high=offsets.activation_offset[1])
        self.piano.set_activation_threshold(offset)

        # Randomize spring stiffness
        key_joints = physics.bind(self.piano.joints)
        offset = [random_state.uniform(low=offsets.key_stiffness[0], high=offsets.key_stiffness[1])] * self.piano.n_keys
        key_joints.stiffness = offset

        # Friction randomization
        offset = random_state.uniform(low=offsets.sliding_friction[0], high=offsets.sliding_friction[1])
        for geom in self.piano.key_geoms:
            physics.bind(geom).friction[0] = offset

    def _apply_joint_randomization(
            self, physics: mjcf.Physics, random_state: np.random.RandomState
    ) -> None:
        # Adjust joints
        damping_offset = self._randomization_config.damping_offset
        for hand in self.hands:
            joints: dict = {j.name: j for j in hand.joints}
            for act in hand.actuators:
                config: JointConfig = self._randomization_config.hand_configuration[act.name]
                offset = random_state.uniform(low=config.stiffness * (1 - damping_offset),
                                              high=config.stiffness * (1 + damping_offset))
                physics.bind(act).gainprm[0] = offset
                if act.name == "forearm_tx":
                    joint_name = act.name
                else:
                    joint_name = act.name.replace("a", "j")

                offset = random_state.uniform(low=config.damping * (1 - damping_offset),
                                              high=config.damping * (1 + damping_offset))
                physics.bind(joints[joint_name]).damping[0] = offset
